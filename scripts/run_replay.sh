...
for bspec in "${BATTERIES[@]}"; do
  ...
  for mspec in "${MODELS[@]}"; do
    ...
    out="data/results/results_rep_${bver}_${mtag}.json"
    [ -f "$out" ] && { echo "[skip] $out exists"; continue; }
    gpu=$((slot % NGPU))
    if [ -n "${pids[$gpu]:-}" ]; then wait "${pids[$gpu]}"; fi
    echo "[launch] gpu=$gpu rep_${bver}_${mtag}"
    CUDA_VISIBLE_DEVICES=$gpu python src/experiments/replay_readout.py --model "$model" \
        --battery "$battery" --out "$out" >"logs/rep_${bver}_${mtag}.log" 2>&1 &
    pids[$gpu]=$!
    slot=$((slot + 1))
  done
done
wait
echo "ALL REPLAY RUNS DONE"
